...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # 3) add a new sentence based on cell and count
        neighbors = set()
        new_count = count
        ...
        # 4) mark any additional cells as safe or mines, repeating until stable
        # 5) infer new sentences via the subset rule (looped until stable too)
        changed = True
        while changed:
            changed = False
            safes = set()
            ...
            # remove any now-empty sentences (cleans up knowledge base)
            self.knowledge = [s for s in self.knowledge if len(s.cells) > 0]

            # infer new sentences via the subset rule
            new_sentences = []
